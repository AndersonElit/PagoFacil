...
SECRET_NAME="pagofacil/dev/fraud-compliance-service"
...
if aws --endpoint-url="$ENDPOINT" secretsmanager describe-secret \
    ...
    aws --endpoint-url="$ENDPOINT" secretsmanager put-secret-value \
        ...
        --secret-string '{"SERVER_PORT": "8083", "R2DBC_URL": "r2dbc:postgresql://${VPS_IP:-localhost}:5432/pagofacil_fraud_compliance_service", "DB_USERNAME": "pagofacil", "DB_PASSWORD": "change_me", "KAFKA_BOOTSTRAP_SERVERS": "${VPS_IP:-localhost}:29092", "KAFKA_CONSUMER_GROUP_ID": "fraud-compliance-service-group"}' \
        --region "$REGION"
