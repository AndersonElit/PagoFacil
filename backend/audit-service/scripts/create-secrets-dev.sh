...
SECRET_NAME="pagofacil/dev/audit-service"
...
if aws --endpoint-url="$ENDPOINT" secretsmanager describe-secret \
    ...
    aws --endpoint-url="$ENDPOINT" secretsmanager put-secret-value \
        ...
        --secret-string '{"SERVER_PORT": "8086", "MONGODB_URI": "mongodb://${VPS_IP:-localhost}:27017/pagofacil_audit_service", "KAFKA_BOOTSTRAP_SERVERS": "${VPS_IP:-localhost}:29092", "KAFKA_CONSUMER_GROUP_ID": "audit-service-group"}' \
        --region "$REGION"
