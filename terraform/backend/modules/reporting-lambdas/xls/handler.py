"""Lambda de formato XLS (DR-6).

Disparada por una rule de EventBridge (detail.format = XLS). Lee el parquet
`processedParquetUri` y escribe el archivo en `output/xls/<reportType>/<reportId>.xlsx`.
"""
import json
import os
import io
...
def _output_key(report_type: str, report_id: str) -> str:
    return f"output/xls/{report_type}/{report_id}.xlsx"
